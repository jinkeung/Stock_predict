...
def get_all_data(stock_code, stock_name):
    ...
    # 종목 코드와 이름 설정
    stock_code = ["035420.KS", "005930.KS"]
    stock_name = ["네이버", "삼성전자"]
    ...
    try:
        ...
        for idx, code in enumerate(stock_code):
            ...
            with connection.cursor() as cursor:
                ...
                CREATE TABLE IF NOT EXISTS {stock_name[idx]} (
            ...
            with connection.cursor() as cursor:
                truncate_table_query = f"TRUNCATE TABLE {stock_name[idx]};"
                cursor.execute(truncate_table_query)
                for index, row in stock_data.iterrows():
                    sql = f"""
                    INSERT INTO {stock_name[idx]} (Date, Open, High, Low, Close, Adj_Close, Volume)
